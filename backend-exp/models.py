...
from pydantic import EmailStr,BaseModel,ConfigDict, ValidationInfo,computed_field,field_validator
...
class User(Base):
    ...
    last_active = Column(DateTime, nullable=True, default=func.now(), onupdate=func.now())# Use utcnow for consistency
...
class DailySparkAnswerVote(Base):
    __tablename__ = "daily_spark_answer_votes"
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    answer_id = Column(Integer, ForeignKey("daily_spark_answers.id"), nullable=False)
    vote_type = Column(Integer, nullable=False)  # +1 for upvote, -1 for downvote
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    user = relationship("User")
    answer = relationship("DailySparkAnswer")
...
class DailySparkAnswerOut(BaseModel): # Pydantic model
    id: int
    question_id: int
    user: str # Directly maps to the 'user' (username string) column
    text: str
    votes: int
    created_at: datetime
    # updated_at: Optional[datetime] = None # If you want to include it
    model_config = ConfigDict(from_attributes=True)


class DailySparkAnswerVoteSchema(BaseModel):
    user_id: int
    answer_id: int
    vote_type: int  # +1 for upvote, -1 for downvote

    model_config = ConfigDict(from_attributes=True)
class DailySparkQuestionOut(BaseModel): # Pydantic model
    id: int
    company: Optional[str] = None
    role: Optional[str] = None
    question: str
    user_id: int # User who POSTED THE QUESTION
    posted_by_username: Optional[str] = None # Username of question poster
    posted_date: date
    answers: List[DailySparkAnswerOut] = [] # List of answers, each using DailySparkAnswerOut
    model_config = ConfigDict(from_attributes=True)

    @field_validator('posted_by_username', mode='before')
    @classmethod
    def populate_posted_by_username(cls, v, info: ValidationInfo):
