...
async def list_users_admin_api(
    ...
    include_inactive: bool = Query(False, description="Set to true to include deactivated users"),
    db: Session = Depends(get_db),
    admin_user: User = Depends(require_admin)
):
    query = db.query(User)
    # Filter out users who are candidates for deactivation from the main list
    # unless explicitly searching or showing inactive users.
    # This prevents them from appearing in two lists by default.
    ninety_days_ago = datetime.now(timezone.utc) - timedelta(days=90)
    if not search and not include_inactive: # Default view: active and not inactive candidates
        query = query.filter(User.is_active == True,
                             or_(User.last_active == None, User.last_active >= ninety_days_ago))


    if not include_inactive and not search : # Only apply if not searching and not explicitly including inactive
         # If neither search term is provided nor include_inactive is true,
         # then exclude users who are active but last_active is older than 90 days.
          query = query.filter(or_(User.is_active == False, User.last_active == None, User.last_active >= ninety_days_ago) )


    if search: # If searching, apply search and include_inactive logic
        query = query.filter(or_(User.username.ilike(f"%{search}%"), User.email.ilike(f"%{search}%")))
        if not include_inactive:
             query = query.filter(User.is_active == True)
    elif not include_inactive: # No search, but only active
         query = query.filter(User.is_active == True)
     # If include_inactive is true and no search, no further activity filter is applied (shows all)


    users = query.order_by(User.username).offset(skip).limit(limit).all()
    return users

# --- NEW ENDPOINT for inactive user candidates ---
@admin_api_router.get("/users/inactive-candidates", response_model=List[UserResponse], summary="List Users Inactive for >90 Days (Admin)")
async def list_inactive_candidates_admin_api(
    db: Session = Depends(get_db),
    admin_user: User = Depends(require_admin) # Ensure only admins can access
):
    logger.info(f"Admin '{admin_user.username}' fetching inactive user candidates (>90 days).")
    # Calculate the date 90 days ago from today (UTC)
    ninety_days_ago = datetime.now(timezone.utc) - timedelta(days=90)

    try:
        # Query for users who are currently active BUT their last_active was before 90 days ago
        # Also include users whose last_active is NULL (e.g. very old accounts before tracking)
        # but ensure they are not admins themselves (admins usually have a reason to be less active).
        inactive_candidates = db.query(User).filter(
            User.is_active == True, # Only consider currently active users
            User.is_admin == False, # Exclude other admins from this list
            or_(
                User.last_active == None, # Include those who never logged in (or before last_active was tracked)
                User.last_active < ninety_days_ago
            )
        ).order_by(User.last_active.asc().nullsfirst(), User.username.asc()).all() # Show oldest first

        logger.info(f"Found {len(inactive_candidates)} inactive candidates.")
        return inactive_candidates
    except Exception as e:
        logger.error(f"Error fetching inactive user candidates: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve inactive user candidates.")
# --- END NEW ENDPOINT ---
...
async def update_user_status_admin_api(
    ...
    user_to_update.updated_at = datetime.now(timezone.utc) # Use timezone-aware UTC
    try:
        db.commit()
...
async def update_user_activity_admin_api(
    user_id: int,
    activity_update: UserActivityUpdate,
    db: Session = Depends(get_db),
    admin_user: User = Depends(require_admin)
):
    ...
    if not activity_update.is_active:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Could not {action} user.")# REMOVE the old @admin_api_router.delete(...) endpoint if it still exists.
