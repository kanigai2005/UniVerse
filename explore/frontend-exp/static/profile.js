...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Example if username is in localStorage
    const storedUsername = localStorage.getItem('username');
    if (storedUsername) {
        window.location.href = `/profile.html?username=${storedUsername}`;
    } else {
        ...
        fetch(`/api/users/${userName}`) // Fetch user data
            ...
            .then(user => {
                if (user) {
                    ...
                    // Store the username in localStorage for subsequent actions on this page
    ...
    // Call the function to fetch and display data if the username is present in the URL
    if (loggedInUserName) {
        ...
        // Optionally redirect to login page if the username is not in the URL
        // window.location.href = '/login.html';
    }
