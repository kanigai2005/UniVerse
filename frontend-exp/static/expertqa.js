...
const BASE_API_PATH_JS = '/api'; // Ensure this matches your Python config.py `BASE_API_PATH`
...
async function fetchMyLikedQuestionIds() {
    ...
    try {
        const response = await fetch(`${BASE_API_PATH_JS}/questions/me/liked`);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initializePage() {
        ...
        await fetchCurrentUserAndUpdateNav(); 
        if (!currentUser) {
    ...
    async function loadSelectedQuestions() { 
        ...
        try {
            const response = await fetch(`${BASE_API_PATH_JS}/expertqa/answerable-questions`); // Corrected
            
            if (!response.ok) {
                let errorDetail = `HTTP error ${response.status}`;
                if (response.status === 404) {
                     errorDetail = "Endpoint for answerable questions not found (404). Please check API routes.";
                } else {
                    try {
            ...
            displayQuestions(questions, selectedQuestionsContainer, false, true); // true for showAnswerButtonForAlumni
        } catch (error) {
    ...
    async function loadPopularQuestions() {
        ...
        try {
            const response = await fetch(`${BASE_API_PATH_JS}/questions/popular`);
            if (!response.ok) throw new Error(`HTTP error ${response.status}`);
            const questions = await response.json();
            if (!Array.isArray(questions)) throw new Error("Invalid data format for popular questions.");
            displayQuestions(questions, communityQuestionsContainer, false, false); // false for showAnswerButtonForAlumni
        } catch (error) {
    ...
    async function loadMyQuestions() {
         ...
         try {
            const response = await fetch(`${BASE_API_PATH_JS}/users/${encodeURIComponent(currentUser.username)}/questions`);
            if (!response.ok) throw new Error(`HTTP error ${response.status}`);
            const myQuestions = await response.json();
            if (!Array.isArray(myQuestions)) throw new Error("Invalid data format for user questions.");
            displayQuestions(myQuestions, myQuestionsContainer, true, false); // false for showAnswerButtonForAlumni
         } catch (error) {
    ...
    function displayQuestions(questions, container, isMyQuestionsList, showAnswerButtonForAlumni) {
        ...
        // Re-attach all dynamic listeners after new content is added
    ...
    function createQuestionElement(question, isMyQuestion, showAnswerButtonForAlumni) {
        ...
        // Answer button shows if this function is told to show it AND current user is alumni
        const displayAnswerBtn = showAnswerButtonForAlumni && currentUser?.is_alumni === true;
    ...
    if (questionForm) {
        questionForm.addEventListener("submit", async function(event) {
            ...
            if(!postButton || !questionInput || !postFeedback) { console.error("Question form submit: missing elements"); return; }
            ...
            try {
                const response = await fetch(`${BASE_API_PATH_JS}/questions`, {
                ...
                postFeedback.textContent = `Error: ${escapeHtml(error.message)}`;
                postFeedback.className = 'post-feedback status-message error';
            } finally {
                setTimeout(() => {
                    postButton.disabled = false;
                    if (postFeedback.className.includes('status-message')) { // Avoid clearing if user typed new msg
    ...
    } else { console.error("Question form (#question-form) not found!"); }
        ...
        container.querySelectorAll('.answer-form').forEach(form => {
            ...
            newForm.addEventListener('submit', async function(event) {
                ...
                if (!textarea || !feedbackDiv || !submitBtn) {
                    console.error("Answer form elements missing for question ID:", questionId); return;
                }
                if (!answerText) {
                ...
                if (!currentUser || currentUser.is_alumni !== true) { // Explicit check
                ...
                try {
                    const response = await fetch(`${BASE_API_PATH_JS}/expertqa/answers/${questionId}`, {
                        method: 'POST',
                    ...
                    setTimeout(() => {
                        if (feedbackDiv.className.includes('status-message')) {
    ...
    function attachLikeAnswerButtonListeners(container) {
     ...
     function appendAnswer(answer, container) { // answer is the newly created answer object from backend
          if (!answer || !container) return;
          const noAnswerMsg = container.querySelector('.no-data');
          if (noAnswerMsg) noAnswerMsg.remove(); // Remove "No answers yet" message

          const answerDiv = document.createElement('div');
          answerDiv.classList.add('answer-item');
          // Assuming the 'answer' object from backend includes 'user.username' or 'username'
          const answererUsername = answer.user?.username || answer.username || currentUser?.username || 'Alumnus';
          ...
          container.appendChild(answerDiv); // Append to the list of answers

          // Re-attach listener for the like button on this newly added answer
    ...
    async function toggleLikeQuestion(questionId, buttonElement) {
        if (!currentUser) { /* Already handled by listener attachment */ return; }
        ...
        try {
            const response = await fetch(`${BASE_API_PATH_JS}/questions/${questionId}/like`, { method: 'POST' });
            ...
            if (data.liked) { // Question is now liked
             ...
             else { alert(`Error: ${escapeHtml(error.message)}`); }
             buttonElement.innerHTML = originalText; // Revert button text
        } finally {
    ...
    async function likeAnswer(answerId, buttonElement) {
        if (!currentUser) { /* Already handled by listener */ return; }
        if (!buttonElement) return;
        buttonElement.disabled = true;
        const originalColor = buttonElement.style.color;
        buttonElement.style.color = '#ccc'; // Visual feedback for processing
        console.log(`Liking answer ${answerId}`);
        try {
            const response = await fetch(`${BASE_API_PATH_JS}/expertqa/answers/${answerId}/like`, { method: 'POST' });
            const data = await response.json();
            ...
            if (countSpan) countSpan.textContent = data.likes;
            buttonElement.style.color = '#28a745'; // Green thumb for success
            // Consider making it permanent for the session if no "unlike answer" feature
            // buttonElement.disabled = true; // if you want to prevent multiple likes from same user in session
        } catch (error) {
            console.error("Error liking answer:", error);
            alert(`Like Error: ${escapeHtml(error.message)}`);
            buttonElement.style.color = originalColor; // Revert color on error
            buttonElement.disabled = false; // Re-enable on error
        } finally {
            // Only re-enable if not successful, or if you allow unliking
            setTimeout(() => {
                if (buttonElement.style.color !== '#28a745' && !buttonElement.disabled) { // Check if it wasn't a successful like
                     buttonElement.disabled = false;
